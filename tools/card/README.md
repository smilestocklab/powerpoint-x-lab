...
**配色は SMILESTOCK.LAB サイトの `:root` をそのまま踏襲しています。**
...
| `--accent` | `#6a4cff` | **背景専用**（バー・ピル・上部帯のグラデーション） |
| `--accent2` | `#3f42ff` | グラデーションの終点 |
| `--accent-soft` | `#a994ff` | **文字用の紫** |

⚠️ **`--accent`（`#6a4cff`）を地の上の文字色に使わないこと。** コントラストが2.57:1しかなくWCAG基準を大きく下回ります。サイト自体も紫は背景として使い、文字には `--accent-soft` を当てています。この使い分けを守ってください（紫背景の上に文字を置くときは白）。

その他の決めごと:

- 角丸16px / ピル型999px / Noto Sans JP も、すべてサイトから継承
